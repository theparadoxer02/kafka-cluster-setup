# Change Ip 10.5.50.233:9092 to inet address of the virtual machine
...
--link zoo-1:zookeeper \
--link kafka-1:kafka \
-e SCHEMA_REGISTRY_KAFKASTORE_CONNECTION_URL=10.5.50.233:2181 \
-e SCHEMA_REGISTRY_HOST_NAME=10.5.50.233 \
...
docker exec -it kafka-1 sh -c " \
  kafka-topics --create --topic quickstart-data --partitions 1 --replication-factor 1 --if-not-exists --zookeeper 10.5.50.233:2181 \
  && kafka-topics --create --topic quickstart-offsets --partitions 1 --replication-factor 1 --if-not-exists --zookeeper 10.5.50.233:2181 \
